...
const FGInternalMovementReport: React.FC = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch(`/api/reports/fg-internal-movement`, {
        method: 'POST',
        headers: {
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'FG Internal Movement');
    const formattedDateTime = DateTime.now().toFormat('yyyy-MM-dd_HH-mm-ss');
    XLSX.writeFile(
      workbook,
      `FG_INTERNAL_MOVEMENT_REPORT_${formattedDateTime}.xlsx`
    );
  ...
  return (
    <div className="space-y-4">
      <Card className="mt-5">
        <CardHeader>
          <CardTitle>FG Internal Movement Report</CardTitle>
        </CardHeader>
        <CardContent>
...
export default FGInternalMovementReport;
