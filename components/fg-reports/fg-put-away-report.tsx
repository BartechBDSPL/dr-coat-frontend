...
const FGPutAwayReport: React.FC = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch(`/api/reports/fg-put-away`, {
        method: 'POST',
        headers: {
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'FG Put Away Report');
    const formattedDateTime = DateTime.now().toFormat('yyyy-MM-dd_HH-mm-ss');
    XLSX.writeFile(workbook, `FG_PUT_AWAY_REPORT_${formattedDateTime}.xlsx`);
  ...
  const exportToPdf = (): void => {
    try {
      ...
      doc.setFontSize(18);
      doc.text('FG Put Away Report', 14, 22);
      ...
      const formattedDateTime = DateTime.now().toFormat('yyyy-MM-dd_HH-mm-ss');
      doc.save(`FG_PUT_AWAY_REPORT_${formattedDateTime}.pdf`);
  ...
  return (
    <div className="space-y-4">
      <Card className="mt-5">
        <CardHeader>
          <CardTitle>FG Put Away Report</CardTitle>
        </CardHeader>
        <CardContent>
...
export default FGPutAwayReport;
