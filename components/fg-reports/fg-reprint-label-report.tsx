...
const FGReprintLabelReport: React.FC = () => {
  ...
  const [reportData, setReportData] = useState<ReprintReportData[]>([]);
  ...
  const filteredData = useMemo(() => {
    return reportData.filter(item => {
      const searchableFields: (keyof ReprintReportData)[] = [
        'production_order_no',
        'item_code',
        'item_description',
        'lot_no',
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch(`/api/reports/fg-reprint-label-report`, {
        method: 'POST',
        headers: {
      ...
      const data: ReprintReportData[] = await response.json();
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(
      workbook,
      worksheet,
      'FG Reprint Label Report'
    );
    const formattedDateTime = DateTime.now().toFormat('yyyy-MM-dd_HH-mm-ss');
    XLSX.writeFile(
      workbook,
      `FG_REPRINT_LABEL_REPORT_${formattedDateTime}.xlsx`
    );
  ...
  const exportToPdf = (): void => {
    try {
      ...
      const columns = [
        ...
        { header: 'Customer No', dataKey: 'customer_no' },
  ...
  return (
    <div className="space-y-4">
      <Card className="mt-5">
        <CardHeader>
          <CardTitle>FG Reprint Label Report</CardTitle>
        </CardHeader>
        <CardContent>
        ...
        <>
          <div className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-5">
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">{stats.totalReprints}</div>
              </CardContent>
            </Card>
...
export default FGReprintLabelReport;
