...
const RMInternalMovementReport: React.FC = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch(`/api/reports/rm-internal-movement`, {
        method: 'POST',
        headers: {
  ...
  const exportToExcel = () => {
    const worksheet = XLSX.utils.json_to_sheet(
      filteredData.map((row, index) => ({
        ...
        'Move By': row.move_by,
        'Move Date': DateTime.fromISO(row.move_date)
          .setZone('GMT')
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'RM Internal Movement');
    const formattedDateTime = DateTime.now().toFormat('yyyy-MM-dd_HH-mm-ss');
    XLSX.writeFile(
      workbook,
      `RM_INTERNAL_MOVEMENT_REPORT_${formattedDateTime}.xlsx`
    );
  ...
  const exportToPdf = (): void => {
    try {
      ...
      const columns = [
        ...
        { header: 'From Location', dataKey: 'from_location' },
        { header: 'To Location', dataKey: 'to_location' },
        { header: 'Move By', dataKey: 'move_by' },
        { header: 'Move Date', dataKey: 'move_date' },
      ];
      ...
      const formattedData = filteredData.map((row, index) => ({
        ...
        move_by: row.move_by,
        move_date: DateTime.fromISO(row.move_date)
          .setZone('GMT')
          .toFormat('yyyy-MM-dd HH:mm:ss'),
      }));

      doc.setFontSize(18);
      doc.text('RM Internal Movement Report', 14, 22);
      ...
      const formattedDateTime = DateTime.now().toFormat('yyyy-MM-dd_HH-mm-ss');
      doc.save(`RM_INTERNAL_MOVEMENT_REPORT_${formattedDateTime}.pdf`);
  ...
  const getDashboardStats = () => {
    ...
    const totalFromLocations = new Set(
      filteredData.map(item => item.from_location)
    ).size;
    const totalToLocations = new Set(filteredData.map(item => item.to_location))
      .size;
  ...
  return (
    <div className="space-y-4">
      <Card className="mt-5">
        <CardHeader>
          <CardTitle>RM Internal Movement Report</CardTitle>
        </CardHeader>
        <CardContent>
                  ...
                  <TableBody>
                    {currentData.map((row, index) => (
                      <TableRow key={index}>
                        ...
                        <TableCell>
                          <div className="flex items-center gap-2">
                            <Badge variant="outline">{row.from_location}</Badge>
                            <ArrowRight className="h-4 w-4 text-muted-foreground" />
                            <Badge variant="outline">{row.to_location}</Badge>
                          </div>
                        </TableCell>
                        <TableCell>{row.move_by}</TableCell>
                        <TableCell>
                          {DateTime.fromISO(row.move_date)
                            .setZone('GMT')
...
export default RMInternalMovementReport;
