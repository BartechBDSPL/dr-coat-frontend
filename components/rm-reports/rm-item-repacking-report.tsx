...
interface ReportData {
  ...
  repacked_by: string;
  repacked_date: string;
}

const RMItemRepackingReport: React.FC = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch(`/api/reports/rm-item-repacking`, {
        method: 'POST',
        headers: {
  ...
  const exportToExcel = () => {
    const worksheet = XLSX.utils.json_to_sheet(
      filteredData.map((row, index) => ({
        'Sr No': index + 1,
        'Repacking Number': row.number,
        'Item Code': row.item_code,
        'Item Description': row.item_description || '-',
        'Lot No': row.lot_no,
        'Serial No': row.serial_no,
        Quantity: row.quantity ?? 0,
        'Repacked Quantity': row.repacked_quantity ?? 0,
        'Repacked By': row.repacked_by,
        'Repacked Date': DateTime.fromISO(row.repacked_date)
          .setZone('GMT')
  ...
  const exportToPdf = (): void => {
    try {
      ...
      const columns = [
        ...
        { header: 'Repacking Number', dataKey: 'repacking_number' },
        ...
        { header: 'Repacked Quantity', dataKey: 'repacked_quantity' },
        { header: 'Repacked By', dataKey: 'repacked_by' },
        { header: 'Repacked Date', dataKey: 'repacked_date' },
      ];

      const formattedData = filteredData.map((row, index) => ({
        srno: index + 1,
        repacking_number: row.number,
        item_code: row.item_code,
        item_description: row.item_description || '-',
        lot_no: row.lot_no,
        serial_no: row.serial_no,
        quantity: row.quantity ?? 0,
        repacked_quantity: row.repacked_quantity ?? 0,
        repacked_by: row.repacked_by,
        repacked_date: DateTime.fromISO(row.repacked_date)
          .setZone('GMT')
          .toFormat('yyyy-MM-dd HH:mm:ss'),
      }));

      doc.setFontSize(18);
      doc.text('RM Item Repacking Report', 14, 22);
      ...
      const formattedDateTime = DateTime.now().toFormat('yyyy-MM-dd_HH-mm-ss');
      doc.save(`RM_ITEM_REPACKING_REPORT_${formattedDateTime}.pdf`);
  ...
  const getDashboardStats = () => {
    ...
    const totalRepackedQuantity = filteredData.reduce(
      (sum, item) => sum + (item.repacked_quantity ?? 0),
      0
    );
  ...
  return (
    <div className="space-y-4">
      <Card className="mt-5">
        <CardHeader>
          <CardTitle>RM Item Repacking Report</CardTitle>
        </CardHeader>
        <CardContent>
        ...
        <>
          <div className="grid grid-cols-2 gap-4 md:grid-cols-5">
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {new Intl.NumberFormat().format(stats.totalQuantity)}
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {new Intl.NumberFormat().format(stats.totalRepackedQuantity)}
                </div>
              </CardContent>
            </Card>
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead className="w-32">Repacking Number</TableHead>
                      <TableHead>Item Code</TableHead>
                      <TableHead>Item Description</TableHead>
                      <TableHead>Lot No</TableHead>
                      <TableHead>Serial No</TableHead>
                      <TableHead>Quantity</TableHead>
                      <TableHead>Repacked Quantity</TableHead>
                      <TableHead>Repacked By</TableHead>
                      <TableHead>Repacked Date</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {currentData.map((row, index) => (
                      <TableRow key={index}>
                        ...
                        <TableCell>
                          {DateTime.fromISO(row.repacked_date)
                            .setZone('GMT')
...
export default RMItemRepackingReport;
