...
const RMItemSplittingReport: React.FC = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch(`/api/reports/rm-item-splitting`, {
        method: 'POST',
        headers: {
  ...
  const exportToExcel = () => {
    const worksheet = XLSX.utils.json_to_sheet(
      filteredData.map((row, index) => ({
        ...
        'Split By': row.split_by,
        'Split Date': DateTime.fromISO(row.split_date)
          .setZone('GMT')
  ...
  const exportToPdf = (): void => {
    try {
      ...
      const columns = [
        ...
        { header: 'Location', dataKey: 'put_location' },
        { header: 'Put Quantity', dataKey: 'put_quantity' },
        { header: 'Split By', dataKey: 'split_by' },
        { header: 'Split Date', dataKey: 'split_date' },
      ];
      ...
      const formattedData = filteredData.map((row, index) => ({
        ...
        put_location: row.put_location,
        put_quantity: row.put_quantity ?? 0,
        split_by: row.split_by,
        split_date: DateTime.fromISO(row.split_date)
          .setZone('GMT')
          .toFormat('yyyy-MM-dd HH:mm:ss'),
      }));

      doc.setFontSize(18);
      doc.text('RM Item Splitting Report', 14, 22);
      ...
      const formattedDateTime = DateTime.now().toFormat('yyyy-MM-dd_HH-mm-ss');
      doc.save(`RM_ITEM_SPLITTING_REPORT_${formattedDateTime}.pdf`);
  ...
  return (
    <div className="space-y-4">
      <Card className="mt-5">
        <CardHeader>
          <CardTitle>RM Item Splitting Report</CardTitle>
        </CardHeader>
        <CardContent>
                  ...
                  <TableBody>
                    {currentData.map((row, index) => (
                      <TableRow key={index}>
                        ...
                        <TableCell>
                          {DateTime.fromISO(row.split_date)
                            .setZone('GMT')
...
export default RMItemSplittingReport;
