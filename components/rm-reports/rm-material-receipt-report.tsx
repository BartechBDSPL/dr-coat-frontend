...
const RMMaterialReceiptReport: React.FC = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch(`/api/reports/rm-material-receipt`, {
        method: 'POST',
        headers: {
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(workbook, `RM_MATERIAL_RECEIPT_${formattedDateTime}.xlsx`);
  ...
  const exportToPdf = (): void => {
    try {
      ...
      doc.setFontSize(18);
      doc.text('RM Material Receipt Report', 14, 22);
      ...
      const formattedDateTime = DateTime.now().toFormat('yyyy-MM-dd_HH-mm-ss');
      doc.save(`RM_MATERIAL_RECEIPT_${formattedDateTime}.pdf`);
  ...
  return (
    <div className="space-y-4">
      <Card className="mt-5">
        <CardHeader>
          <CardTitle>RM Material Receipt Report</CardTitle>
        </CardHeader>
        <CardContent>
...
export default RMMaterialReceiptReport;
