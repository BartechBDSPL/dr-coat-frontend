...
const RMPurchaseReturnReport: React.FC = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch(`/api/reports/rm-purchase-return`, {
        method: 'POST',
        headers: {
  ...
  const exportToPdf = (): void => {
    try {
      ...
      const columns = [
        ...
        { header: 'Return Reason', dataKey: 'return_reason' },
      ];
      ...
      doc.setFontSize(18);
      doc.text('RM Purchase Return Report', 14, 22);
      ...
      const formattedDateTime = DateTime.now().toFormat('yyyy-MM-dd_HH-mm-ss');
      doc.save(`RM_PURCHASE_RETURN_REPORT_${formattedDateTime}.pdf`);
  ...
  return (
    <div className="space-y-4">
      <Card className="mt-5">
        <CardHeader>
          <CardTitle>RM Purchase Return Report</CardTitle>
        </CardHeader>
        <CardContent>
                  ...
                  <TableBody>
                    {currentData.map((row, index) => (
                      <TableRow key={index}>
                        ...
                        <TableCell>{row.return_reason}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
...
export default RMPurchaseReturnReport;
