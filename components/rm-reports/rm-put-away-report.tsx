...
const RMPutAwayReport: React.FC = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch(`/api/reports/rm-put-away`, {
        method: 'POST',
        headers: {
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'RM Put Away Report');
    const formattedDateTime = DateTime.now().toFormat('yyyy-MM-dd_HH-mm-ss');
    XLSX.writeFile(workbook, `RM_PUT_AWAY_REPORT_${formattedDateTime}.xlsx`);
  ...
  const exportToPdf = (): void => {
    try {
      ...
      doc.setFontSize(18);
      doc.text('RM Put Away Report', 14, 22);
      ...
      const formattedDateTime = DateTime.now().toFormat('yyyy-MM-dd_HH-mm-ss');
      doc.save(`RM_PUT_AWAY_REPORT_${formattedDateTime}.pdf`);
  ...
  return (
    <div className="space-y-4">
      <Card className="mt-5">
        <CardHeader>
          <CardTitle>RM Put Away Report</CardTitle>
        </CardHeader>
        <CardContent>
...
export default RMPutAwayReport;
