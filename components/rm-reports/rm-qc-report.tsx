...
const RMQCReport: React.FC = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch(`/api/reports/rm-qc`, {
        method: 'POST',
        headers: {
  ...
  const exportToExcel = () => {
    const worksheet = XLSX.utils.json_to_sheet(
      filteredData.map((row, index) => ({
        ...
        'QC By': row.qc_by,
        'QC Date': DateTime.fromISO(row.qc_date)
          .setZone('GMT')
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'RM QC Report');
    const formattedDateTime = DateTime.now().toFormat('yyyy-MM-dd_HH-mm-ss');
    XLSX.writeFile(workbook, `RM_QC_REPORT_${formattedDateTime}.xlsx`);
  ...
  const exportToPdf = (): void => {
    try {
      ...
      const formattedData = filteredData.map((row, index) => ({
        ...
        qc_by: row.qc_by,
        qc_date: DateTime.fromISO(row.qc_date)
          .setZone('GMT')
          .toFormat('yyyy-MM-dd HH:mm:ss'),
        reason: row.reason || '-',
      }));
      ...
      const formattedDateTime = DateTime.now().toFormat('yyyy-MM-dd_HH-mm-ss');
      doc.save(`RM_QC_REPORT_${formattedDateTime}.pdf`);
  ...
  const getDashboardStats = () => {
    ...
    const totalPass = filteredData.filter(
      item => item.qc_status?.toLowerCase() === 'pass'
    ).length;
    const totalFail = filteredData.filter(
      item => item.qc_status?.toLowerCase() === 'fail'
    ).length;
  ...
  return (
    <div className="space-y-4">
      <Card className="mt-5">
        <CardHeader>
          <CardTitle>RM QC Report</CardTitle>
        </CardHeader>
        <CardContent>
                    ...
                    {currentData.map((row, index) => (
                      <TableRow key={index}>
                        ...
                        <TableCell>
                          <Badge
                            variant={
                              row.qc_status?.toLowerCase() === 'reject'
                                ? 'destructive'
                                : 'default'
                            }
                            className={
                              row.qc_status?.toLowerCase() === 'approve'
