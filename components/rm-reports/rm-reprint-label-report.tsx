...
const RMReprintLabelReport: React.FC = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch(`/api/reports/rm-reprint-label-report`, {
        method: 'POST',
        headers: {
  ...
  const exportToExcel = () => {
    const worksheet = XLSX.utils.json_to_sheet(
      filteredData.map((row, index) => ({
        ...
        'Finished Quantity': row.finished_quantity ?? 0,
        UOM: row.uom,
  ...
  const exportToPdf = (): void => {
    try {
      const doc = new jsPDF('l', 'mm', 'a2') as any;
      const columns = [
        ...
        { header: 'Vendor Code', dataKey: 'vendor_code' },
        { header: 'Vendor Name', dataKey: 'vendor_name' },
        { header: 'Fin. Qty', dataKey: 'finished_quantity' },
        ...
        { header: 'Reason', dataKey: 'reprint_reason' },
      ...
      const formattedData = filteredData.map((row, index) => ({
        ...
        finished_quantity: row.finished_quantity ?? 0,
        uom: row.uom,
        quantity: row.quantity ?? 0,
      ...
      doc.setFontSize(18);
      doc.text('RM Reprint Label Report', 14, 22);
      ...
      const formattedDateTime = DateTime.now().toFormat('yyyy-MM-dd_HH-mm-ss');
      doc.save(`RM_REPRINT_LABEL_REPORT_${formattedDateTime}.pdf`);
  ...
  return (
    <div className="space-y-4">
      <Card className="mt-5">
        <CardHeader>
          <CardTitle>RM Reprint Label Report</CardTitle>
        </CardHeader>
        <CardContent>
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead>Fin. Qty</TableHead>
                      <TableHead>UOM</TableHead>
                      <TableHead>Quantity</TableHead>
                      <TableHead>Serial No</TableHead>
                      <TableHead>Base Wt</TableHead>
...
export default RMReprintLabelReport;
