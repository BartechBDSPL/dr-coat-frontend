...
const RMStockAdjustmentReport: React.FC = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch(`/api/reports/rm-stock-adjustment`, {
        method: 'POST',
        headers: {
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'RM Stock Adjustment');
    const formattedDateTime = DateTime.now().toFormat('yyyy-MM-dd_HH-mm-ss');
    XLSX.writeFile(
      workbook,
      `RM_STOCK_ADJUSTMENT_REPORT_${formattedDateTime}.xlsx`
    );
  ...
  const exportToPdf = (): void => {
    try {
      ...
      doc.setFontSize(18);
      doc.text('RM Stock Adjustment Report', 14, 22);
      ...
      const formattedDateTime = DateTime.now().toFormat('yyyy-MM-dd_HH-mm-ss');
      doc.save(`RM_STOCK_ADJUSTMENT_REPORT_${formattedDateTime}.pdf`);
  ...
  return (
    <div className="space-y-4">
      <Card className="mt-5">
        <CardHeader>
          <CardTitle>RM Stock Adjustment Report</CardTitle>
        </CardHeader>
        <CardContent>
...
export default RMStockAdjustmentReport;
