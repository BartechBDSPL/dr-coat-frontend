...
const RMStockTakeReport: React.FC = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch(`/api/reports/rm-stock-take`, {
        method: 'POST',
        headers: {
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'RM Stock Take Report');
    const formattedDateTime = DateTime.now().toFormat('yyyy-MM-dd_HH-mm-ss');
    XLSX.writeFile(workbook, `RM_STOCK_TAKE_REPORT_${formattedDateTime}.xlsx`);
  ...
  const exportToPdf = (): void => {
    try {
      ...
      doc.setFontSize(18);
      doc.text('RM Stock Take Report', 14, 22);
      ...
      const formattedDateTime = DateTime.now().toFormat('yyyy-MM-dd_HH-mm-ss');
      doc.save(`RM_STOCK_TAKE_REPORT_${formattedDateTime}.pdf`);
  ...
  return (
    <div className="space-y-4">
      <Card className="mt-5">
        <CardHeader>
          <CardTitle>RM Stock Take Report</CardTitle>
        </CardHeader>
        <CardContent>
...
export default RMStockTakeReport;
