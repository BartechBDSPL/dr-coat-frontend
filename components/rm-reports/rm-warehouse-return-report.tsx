...
const RMWarehouseReturnReport: React.FC = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch(`/api/reports/rm-warehouse-return`, {
        method: 'POST',
        headers: {
  ...
  const exportToExcel = () => {
    const worksheet = XLSX.utils.json_to_sheet(
      filteredData.map((row, index) => ({
        'Sr No': index + 1,
        'Stock Transfer No': row.stock_transfer_number,
  ...
  const exportToPdf = (): void => {
    try {
      ...
      const columns = [
        ...
        { header: 'Stock Transfer No', dataKey: 'stock_transfer_number' },
      ...
      doc.setFontSize(18);
      doc.text('RM Warehouse Return Report', 14, 22);
      ...
      const formattedDateTime = DateTime.now().toFormat('yyyy-MM-dd_HH-mm-ss');
      doc.save(`RM_WAREHOUSE_RETURN_REPORT_${formattedDateTime}.pdf`);
  ...
  return (
    <div className="space-y-4">
      <Card className="mt-5">
        <CardHeader>
          <CardTitle>RM Warehouse Return Report</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
            <div>
              <Label htmlFor="stockTransferNumber">Stock Transfer No</Label>
              <Input
                id="stockTransferNumber"
                value={stockTransferNumber}
                onChange={e => setStockTransferNumber(e.target.value)}
                placeholder="Enter Stock Transfer No"
              />
            </div>
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead>Stock Transfer No</TableHead>
...
export default RMWarehouseReturnReport;
