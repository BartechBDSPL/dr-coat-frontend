...
const RMExistingDataUpload: React.FC = () => {
  ...
  const fetchItemCodes = async () => {
    ...
    try {
      const { data } = await axios.get('/api/rm-existing-data/get-item-codes', {
        params: { t: Date.now() },
      ...
      console.error('Error fetching RM item codes:', error);
    } finally {
  ...
  const fetchLotNumbers = async (itemCode: string) => {
    ...
    try {
      const { data } = await axios.post(
        '/api/rm-existing-data/get-lot-numbers',
        { item_code: itemCode },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      if (data.Status === 'T') {
      ...
      console.error('Error fetching RM lot numbers:', error);
    } finally {
  ...
  const fetchDetails = async (itemCode: string, lotNo: string) => {
    try {
      const { data } = await axios.post(
        '/api/rm-existing-data/get-details',
        { item_code: itemCode, lot_no: lotNo },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      if (data.Status === 'T') {
  ...
  const fetchWarehouses = async () => {
    ...
    try {
      const { data } = await axios.get('/api/hht/warehouse-codes');
      if (Array.isArray(data)) {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      reader.onloadstart = () => setIsLoadingFile(true);
      reader.onload = () => {
  ...
  const handleUpload = async () => {
    ...
    formData.append('uploaded_by', getUserID() || '');
    formData.append('print_status', 'Open');

    try {
      const { data } = await axios.post(
        '/api/rm-existing-data/upload-excel',
        formData,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      if (data.Status === 'T') {
  ...
  const generateSerialNumbers = async () => {
    ...
    try {
      const { data } = await axios.post(
        '/api/rm-existing-data/find-serial-number',
        { item_code: selectedItemCode, lot_no: selectedLotNo },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      ...
      for (let i = 0; i < numFullLabels; i++) {
        ...
        generatedSerials.push({ serialNo, qty: baseWeightValue });
      }
  ...
  const handlePrintLabels = async () => {
    ...
    if (!validateTotalQuantity()) return;
    ...
    if (missingFields.length > 0) {
      ...
      switch (firstInvalid) {
        case 'itemCode':
          itemCodeRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
          itemCodeRef.current?.focus();
          break;
        case 'lotNo':
          lotNoRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
          lotNoRef.current?.focus();
          break;
        case 'baseWeight':
          baseWeightRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
          baseWeightRef.current?.focus();
          break;
        case 'tareWeight':
          tareWeightRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
          tareWeightRef.current?.focus();
          break;
        case 'totalQty':
          totalQtyRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
          totalQtyRef.current?.focus();
          break;
        case 'mfgDate':
          mfgDateRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
          mfgDateRef.current?.focus();
          break;
        case 'expDate':
          expDateRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
        ...
        case 'printer':
          ...
          break;
        case 'putLocation':
          putLocationRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
  ...
  const confirmPrintLabels = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        vendor_code: itemDetails?.vendor_code || '',
        vendor_name: itemDetails?.vendor_name || '',
        serial_no: serialNo,
      ...
      const { data } = await axios.post(
        '/api/rm-existing-data/insert-label-printing',
        payload,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const downloadSampleFile = () => {
    ...
    link.href = '/exisiting_sample_data_rm.xlsx';
    link.download = 'exisiting_sample_data_rm.xlsx';
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            RM Existing Data Label Printing
          </CardTitle>
        </CardHeader>
        ...
        <Card>
          ...
          <CardContent>
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
              ...
              {itemDetails.vendor_code && (
                <div className="space-y-2">
                  <Label htmlFor="vendor_code">Vendor Code</Label>
                  <Input
                    id="vendor_code"
                    value={itemDetails.vendor_code}
                    disabled
                  />
                </div>
              )}
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead>
                        Base Weight <span className="text-red-500">*</span>
                      </TableHead>
                      <TableHead>
                        Tare Weight <span className="text-red-500">*</span>
                      </TableHead>
        ...
        >
          ...
          <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
            <div className="space-y-1">
              ...
              <p className="text-base font-medium">{baseWeight} KG</p>
            </div>
            <div className="space-y-1">
              <p className="text-sm text-muted-foreground">Tare Weight</p>
              <p className="text-base font-medium">{tareWeight} KG</p>
            </div>
            <div className="space-y-1">
              <p className="text-sm text-muted-foreground">Total Weight</p>
              <p className="text-base font-medium">{totalWeight} KG</p>
            </div>
            <div className="space-y-1">
          ...
          <div className="mt-4 rounded-md bg-muted p-3">
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 md:grid-cols-3">
              ...
              <div className="space-y-0">
                ...
                <p className="font-medium">{totalQty} KG</p>
              </div>
              {itemDetails?.vendor_code && (
                <div className="space-y-0">
                  <p className="text-xs text-muted-foreground">Vendor Code</p>
                  <p className="font-medium">{itemDetails.vendor_code}</p>
                </div>
              )}
              {itemDetails?.vendor_name && (
                <div className="space-y-0">
                  <p className="text-xs text-muted-foreground">Vendor Name</p>
                  <p className="font-medium">{itemDetails.vendor_name}</p>
                </div>
              )}
              <div className="space-y-0">
...
export default RMExistingDataUpload;
