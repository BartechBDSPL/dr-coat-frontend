...
const RMReprintApproval: React.FC = () => {
  const [selectedRequest, setSelectedRequest] =
    useState<RMReprintRequest | null>(null);
  const [pendingRequests, setPendingRequests] = useState<RMReprintRequest[]>(
    []
  );
  ...
  const fetchPendingRequests = async () => {
    try {
      ...
      const response = await axios.post(
        '/api/rm/rm-reprint-request-pending',
        {}
      );
  ...
  const handleSelect = (request: RMReprintRequest) => {
  ...
  const handleApproval = async () => {
    ...
    try {
      const response = await axios.post('/api/rm/rm-reprint-request-update', {
  ...
  const handleRejection = async () => {
    ...
    try {
      const response = await axios.post('/api/rm/rm-reprint-request-reject', {
        sr_no: selectedRequest.sr_no,
        reject_by: getUserID(),
      });
  ...
  const filteredData = useMemo(() => {
    return pendingRequests.filter(item => {
      const searchableFields: (keyof RMReprintRequest)[] = [
        'request_by',
        'serial_no',
        'status',
      ];
    ...
    <Card className="border-border shadow-sm">
      <CardContent className="flex flex-col items-center justify-center py-16">
        ...
        <p className="max-w-md text-center text-muted-foreground">
          There are currently no pending RM reprint requests to display.
        </p>
      </CardContent>
    </Card>
  );
...
export default RMReprintApproval;
