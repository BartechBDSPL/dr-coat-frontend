...
const RMReprintRequest: React.FC = () => {
  ...
  const [reportData, setReportData] = useState<RMReprintData[]>([]);
  ...
  const filteredData = useMemo(() => {
    return reportData.filter(item => {
      const searchableFields: (keyof RMReprintData)[] = [
        'grn_no',
        'item_code',
        'item_description',
        'lot_no',
  ...
  const fetchApprovedRequests = async () => {
    ...
    try {
      const response = await fetch(
        '/api/rm/rm-reprint-request-printing-pending',
        {
          method: 'POST',
          headers: {
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch(`/api/rm/rm-reprint-label-details`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          authorization: `Bearer ${token}`,
      ...
      if (!response.ok) throw new Error('Network response was not ok');
      const data: RMReprintData[] = await response.json();
  ...
  const handleReprintConfirm = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/rm/rm-reprint-request-insert`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          authorization: `Bearer ${token}`,
  ...
  const handlePrintConfirm = async () => {
    ...
    try {
      const response = await fetch('/api/rm/rm-reprint-label-insert', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          authorization: `Bearer ${token}`,
  ...
  const getDashboardStats = () => {
    const totalGRNs = new Set(reportData.map(item => item.grn_no)).size;
    ...
    const totalVendors = new Set(reportData.map(item => item.vendor_code)).size;
  ...
  return (
    <div className="space-y-4">
      <Card className="mt-5">
        <CardHeader>
          <CardTitle>RM Reprint Request</CardTitle>
        </CardHeader>
        <CardContent>
      ...
      <Dialog open={showReprintDialog} onOpenChange={setShowReprintDialog}>
        <DialogContent className="sm:max-w-[600px]">
          <DialogHeader>
            <DialogTitle>Submit RM Reprint Request</DialogTitle>
            <DialogDescription>
      ...
      <Dialog open={showPrintDialog} onOpenChange={setShowPrintDialog}>
        <DialogContent className="sm:max-w-[600px]">
          <DialogHeader>
            <DialogTitle>Print RM Labels</DialogTitle>
            <DialogDescription>
...
export default RMReprintRequest;
